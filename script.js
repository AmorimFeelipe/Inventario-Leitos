        ...
        // Função para migrar dados antigos: extrair nome após " - "
                    ...
                    snap.forEach(d => {
                        ...
                        if (data.inventory) {
                            data.inventory = data.inventory.map(item => ({
                                ...item,
                                type: migrateItemType(item.type)
                            }));
                        }
                        units.push({ id: d.id, ...data });
                    });
                    render();
        ...
        window.openEditModal = (unit) => {
            ...
            // Estrutura simplificada: apenas nome + quantidades
            unit.inventory.forEach(item => {
                const nome = migrateItemType(item.type); // Migrar se necessário
                const cad = item.cadastrados || item.quantity || 0;
                const naoCad = item.naoCadastrados || 0;
                addBedRowSimple(nome, cad, naoCad);
            });
        ...
        // --- FORM & SAVING ---
        // Função simplificada para adicionar linha de leito (nome + cad + não cad)
        function addBedRowSimple(nome = '', cad = 0, naoCad = 0) {
            const row = document.createElement('div');
            row.className = 'sub-bed-row';
            row.innerHTML = `
                <input type="text" class="form-input sub-bed-name" placeholder="Nome do leito (ex: Alcon)" value="${nome}">
                <div class="number-field">
                    <label class="number-label">Cadastrados</label>
                    <input type="number" class="form-input bed-cad" placeholder="0" value="${cad}" min="0" onfocus="this.select()">
                </div>
                <div class="number-field">
                    <label class="number-label">Não Cad.</label>
                    <input type="number" class="form-input bed-naocad" placeholder="0" value="${naoCad}" min="0" onfocus="this.select()">
                </div>
                <button type="button" class="remove-bed-btn" title="Remover">&times;</button>
            `;
            row.querySelector('.remove-bed-btn').onclick = () => row.remove();
            bedInputs.appendChild(row);
        }
        ...
        form.addEventListener('submit', async (e) => {
            ...
            // Estrutura simplificada: processar todas as linhas de leito
            document.querySelectorAll('.sub-bed-row').forEach(row => {
                const nomeLeito = row.querySelector('.sub-bed-name').value.trim();
                const cadastrados = parseInt(row.querySelector('.bed-cad')?.value) || parseInt(row.querySelectorAll('.form-input')[1]?.value) || 0;
                const naoCadastrados = parseInt(row.querySelector('.bed-naocad')?.value) || parseInt(row.querySelectorAll('.form-input')[2]?.value) || 0;
                const total = cadastrados + naoCadastrados;

                if (nomeLeito && total > 0) {
                    newInv.push({
                        type: nomeLeito, // Salvar apenas o nome, sem tipo
                        quantity: total,
        ...
        document.getElementById('add-bed-input').onclick = () => addBedRowSimple();
        ...
        document.getElementById('unit-template').onchange = (e) => {
            ...
            if (templateId && templatesUnidades[templateId]) {
                ...
                // Estrutura simplificada: apenas linhas de leitos
                bedInputs.innerHTML = '';
                // Adicionar uma linha vazia inicial para o usuário preencher
                addBedRowSimple();
            } else {
                ...
                bedInputs.innerHTML = '';
                addBedRowSimple(); // Linha vazia para começar
            }
        };

        // Função removida - não mais necessária com estrutura simplificada
        ...
        document.getElementById('add-unit-btn').onclick = () => {
            ...
            // Adicionar uma linha inicial para começar
            addBedRowSimple();
